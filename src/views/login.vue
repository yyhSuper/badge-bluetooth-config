...
import {fetchMenuData} from "../router";
...
export default {
  ...
  // 创建完毕状态(里面是操作)
  created() {
    let token = 'eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJleHAiOjE2ODM5OTMyNTcsInVzZXJJZCI6IjEiLCJ1c2VybmFtZSI6IjE1MDEyNzk5NzQ3In0.1A9aMRjBP5pRUBAV4iaGAXx4oGkrL9CBUjxxAdsv2cU'.split(".")
    let str=token[1]
  ...
  methods: {
    ...
    async submitForm(formName) {
        ...
        if (res.result === '200') {
          ...
          await fetchMenuData(store,'login.vue'); // 确保传递了 store 对象
          // 设置登录状态并跳转
          this.logining = false;
